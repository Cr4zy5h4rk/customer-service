...
export default class Navbar implements OnInit {
  ...
  customerEntityNavbarItems = signal<NavbarItem[] | null>(null);
  ...
  loadMicrofrontendsEntities(): void {
    ...
    loadNavbarItems('customer').then(
      items => {
        this.customerEntityNavbarItems.set(items);
      },
      (error: unknown) => {
        // eslint-disable-next-line no-console
        console.log('Error loading customer entities', error);
      },
    );
  }
}
